...
// fonctions publiques
	function sizeOfText($texte, $largeur)
	{
		...
		// Assurez-vous que la largeur n'est pas nulle pour éviter la division par zéro
		if ($largeur <= 0) {
			return 0;
		}
		...
		while ($loop)
		{
			...
			// Assurez-vous que la largeur n'est pas nulle pour éviter la division par zéro
			$res = ($largeur > 0) ? 1 + floor($length / $largeur) : 0;
...
*/function addLine($ligne, $tab)
{
	...
	foreach ($colonnes as $lib => $pos) {
		$longCell = $pos - 2;
		$texte = isset($tab[$lib]) ? utf8_decode($tab[$lib]) : ''; // Convertir en UTF-8
		$length = $this->GetStringWidth($texte);
		$tailleTexte = $this->sizeOfText($texte, $length);

		// Vérifier si la taille du texte est non nulle avant d'ajuster $maxSize
		if ($tailleTexte > 0) {
			$maxSize = max($maxSize, $this->GetY());
		}
